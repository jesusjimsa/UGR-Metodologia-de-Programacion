...
using namespace std;
	    ...
	    long long maximo = filas * columnas;

	    nfilas = filas;
            ncolumnas = columnas;

            datos = new byte*[nfilas];
            datos[0] = new byte[maximo];
        ...
        Imagen Imagen::concatenar(const Imagen & a, const Imagen & b){
            int filas, columnas;
            destruir();
        ...
        bool Imagen::escribirImagen(const char nombreFichero[], bool esBinario){
            ...
            if (esBinario)
                escribir = escribirPGMBinario(nombreFichero, datos[0], nfilas, ncolumnas);
            else
                escribir = escribirPGMTexto(nombreFichero, datos[0], nfilas, ncolumnas);
        ...
        bool Imagen::aArteASCII (const char grises[], char arteASCII[], int maxlong){
            ...
            if((ncolumnas + 1) * nfilas <= maxlong){
                for(int i = 0; i <= max_valor; i++){
                    arteASCII[total_utilizados] = grises[getPos(i) * cardinal / 256];
                    total_utilizados++;
                    contador++;
